...
# /tmp/machgate-ios-parallel-logs/, summary table in
# /tmp/machgate-ios-parallel-results.txt
set -uo pipefail
...
default_ct_dir=""
for candidate in \
    "$machgate_root/../game-engine-machgate/build/buck2/common-tests-ios-arm64-release" \
    "$machgate_root/../../game-engine-machgate/build/buck2/common-tests-ios-arm64-release" \
    "/home/coder/git/roblox/game-engine-machgate/build/buck2/common-tests-ios-arm64-release"; do
    if [ -d "$candidate" ]; then
...
LOG_DIR="/tmp/machgate-ios-parallel-logs"
mkdir -p "$LOG_DIR"
RESULTS_FILE="/tmp/machgate-ios-parallel-results.txt"
LOCK_FILE="/tmp/machgate-ios-parallel-results.lock"
